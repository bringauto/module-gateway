...
from .modules import create_connect_response, check_supported_device_for_module, ModuleType
...
class Server:
    ...
    def __init__(self, client_exc: ClientException) -> None:
        ...
        self.connected_devices = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: DeviceItem())))
            ...
            async with client.messages() as messages:
                ...
                async for message in messages:
                    ...
                    if self.connected_devices[d_module][d_type][d_role].priority == Server.DISCONNECTED:
                        logging.warning(
                            f"Message for not connected device: {d_module}/{d_type}/{d_role}\
                                          has been received"
                        )
                        continue
    ...
    async def _handle_client(self, reader: asyncio.streams.StreamReader, writer: asyncio.streams.StreamWriter) -> None:
        ...
        internal_client_msg = internal_protocol.InternalClient.FromString(data)
        try:
            await self._init_sequence(internal_client_msg, writer)
        except Exception:
            return

        device = internal_client_msg.deviceConnect.device
        ...
        while True:
            ...
            if self.connected_devices[d_module][d_type][d_role].priority != priority:
                logging.info(f"Device {d_name} will be disconnected, because device with higher priority was connected")
                break
            data = await self._receive_message(reader)
            if not data:
                break
    ...
    async def _init_sequence(
        self, internal_client_msg: internal_protocol.InternalClient, writer: asyncio.streams.StreamWriter
    ) -> None:
        if not internal_client_msg.HasField("deviceConnect"):
            logging.warning(f"First received message is not DeviceConnect")
            raise Exception()
        connect = internal_client_msg.deviceConnect
        ...
        if self.client_exc is not ClientException.ServerTookTooLong:
            await self._send_message(writer, create_connect_response(response_type))
